...
int main() {
	...
	uint32_t TamPreambulo = floor( TotalReads/2 );
	if( TotalReads%2 != 0 ) TamPreambulo = TamPreambulo+1;
...
void Inst2Bin(  uint8_t *BinInst, uint8_t *Preambulos, uint32_t *posBInst, uint32_t *posPream, 
	...
	if ( (*flagPream) == 0 ) {
		...
		#if TEST_PRE 
			fprintf(PREAMBULOS,"Strand: %c , Preambulo: %"PRIu8", Position: %"PRIu32" \n", strand,Preambulos[auxPosPream],auxPosPream);
		#endif
		(*flagPream) = 1;
	} else {
...
uint8_t BitsBase(uint8_t BRead, uint8_t BRef, FILE *ELOGS){
	...
	if ((BRead!=0x9)&&(BRef!=0x9)){
		...
		if (BRead>BRef){
			...
			if (BRead==BRef) {
				#if ERROR_LOG
					fprintf(ELOGS," A Error Grave entre bases iguales Base1 %"PRIu8" Base2 %"PRIu8" \n", BRead,  BRef);
				#endif
			} else {
		...
		switch(auxInt){
			case 0:
				#if ERROR_LOG  
					fprintf(ELOGS," B Error Grave entre bases iguales Base1 %"PRIu8" Base2 %"PRIu8" \n", BRead,  BRef);
				#endif
			break;
